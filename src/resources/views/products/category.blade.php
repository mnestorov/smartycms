	...
	<div class="admin-content">
		<form action="shop/categories/save/{{ $category->id or 'new'}}" method="post" enctype="multipart/form-data">
			...
			<select name="parent_id">
				...
				@if ($category->id)
					@foreach (\SmartyStudio\SmartyCms\Models\ProductCategory::whereNotIn('id', $category->children->pluck('id')->push($category->id))->get() as $category_option)
						<option value="{{$category_option->id}}" {{$category_option->id==$category->parent_id ? 'selected' : ''}}>{{$category_option->title}}</option>
					@endforeach
				@else
					@foreach (\SmartyStudio\SmartyCms\Models\ProductCategory::all() as $category_option)
