	...
	<div class="admin-content">
		...
		<form action="/{{Request::segment(1)}}/shop/orders" class="filters" {!!empty(Request::input('filter')) ? '' : 'style="display:block;"'!!}>
			...
			--}}<input type="text" name="filter[total_price]" placeholder="Price"
				...
				<select name="filter[order_status_id]" placeholder="Status">
					...
					@foreach (SmartyStudio\SmartyCms\Models\OrderStatus::all() as $status)
		...
		@if (!empty($orders->first()))
			...
			@foreach ($orders as &$order)
				@if ($order->status->id == SmartyStudio\SmartyCms\Models\OrderStatus::DELIVERED)
					<tr class='blue'>
				@elseif ($order->status->id == SmartyStudio\SmartyCms\Models\OrderStatus::CREATED)
					<tr class='green'>
				@elseif ($order->status->id == SmartyStudio\SmartyCms\Models\OrderStatus::NOT_ACCEPTED)
					<tr class='red'>
				@else
					<tr>
				@endif
					...
					<td>{{$order->total_price + $order->total_price * (empty(config('smartycms.invoice.vat')) ? '0' : config('smartycms.invoice.vat'))/100 }} {{$order->currency}}</td>
